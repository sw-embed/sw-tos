...
output=$($EMU --load-binary "$OUT_DIR/program.bin@0" --entry 0 \
    -u '1x23\xFF\x01\xF4\x01\x00\xFF\x01\x58\x02\x00\xFF\x01\xBC\x02\x00\x1B4\xFF\x02\xC0\xFF\x03\x45\xFF\x02\x24\x1E\x45\xFF\x02\x88\x1E\x45\x1B506' \
...
menu_count=$(grep -o 'MENU 1=Hello' <<<"$output" | wc -l | tr -d ' ')
