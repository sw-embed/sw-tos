...
read -r image_ordinal record_offset image_offset image_length < <(
...
for corruption in magic entry size; do
    ...
    if [ "$corruption" = magic ]; then
        printf 'X' | dd of="$bad_media" bs=1 seek="$image_offset" conv=notrunc 2>/dev/null
    elif [ "$corruption" = entry ]; then
        printf '\377\377\377' | dd of="$bad_media" bs=1 seek="$((image_offset + 18))" conv=notrunc 2>/dev/null
    else
...
# Resolve the selected record rather than depending on catalog ordering.
for corruption in ordinal alignment length flags bounds; do
    bad_media="$OUT_DIR/corrupt-extent-$corruption.bin"
    cp "$MEDIA" "$bad_media"
    case "$corruption" in
        ordinal) printf '\377' | dd of="$bad_media" bs=1 seek="$((record_offset + 16))" conv=notrunc 2>/dev/null ;;
        alignment) printf '\201' | dd of="$bad_media" bs=1 seek="$((record_offset + 19))" conv=notrunc 2>/dev/null ;;
        length) printf '\000\000\045' | dd of="$bad_media" bs=1 seek="$((record_offset + 20))" conv=notrunc 2>/dev/null ;;
...
for corruption in count name; do
    ...
    if [ "$corruption" = count ]; then
        printf "\\$(printf '%03o' "$((catalog_count - 1))")" | dd of="$bad_media" bs=1 seek=0 conv=notrunc 2>/dev/null
    else
