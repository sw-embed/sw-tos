...
mkdir -p "$OUT_DIR"
"$META_GEN" prep "$ROOT_DIR/hal/cor24/protocol-v1.s" \
...
output=$($EMU --lgo "$OUT_DIR/protocol-v1.lgo" \
    -u 'junk\xA5\x5A\x01\x01\x02\x02\x00\xA5\x5A\x05\xA5\x5A\x01\x01\x00\x00\x00\x03\xA5\x5A\x01\xFE\x00\x00\x00\xFF\xA5\x5A\x01\x07\x00\x03\x00\x01\x00\x00\x0C' \
    --speed 0 -n 1000000 --quiet 2>/dev/null)

if [ "$output" != 'A2EEG3' ]; then
    echo "FAIL: target protocol decoder produced '$output', expected 'A2EEG3'" >&2
    exit 1
fi
